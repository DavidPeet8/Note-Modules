...
namespace Preprocessor
{
	...
	globalArgs(a),
	fileList{a.getMap()},
	baseNotesDir{a.getBaseNotesPath()}
	{}
	...
	void Preprocessor::startBuild()
	{
		...
		linkBuiltFiles(fs::absolute(baseNotesDir + "/build"), ""); // Must pass absolute path
	}
	...
	void Preprocessor::build(const string &noteName)
	{
		...
		// std::bind(&Preprocessor::includeHandler, this, note, ref(out), _1), - class member example
		Cmd::CommandHandlers handlers = {
			bind(includeHandler, note, ref(out), this, _1),
			bind(linkHandler, note, ref(out), _1), 
			bind(nobuildHandler, note, ref(out), _1),
			bind(imgHandler, note, ref(out), _1),
	...
	void Preprocessor::linkBuiltFiles(const string &baseBuildPath, const string &pathTail)
	{
		...
		for (const auto &item : baseDir)
		{
			...
			if (item.is_directory())
			{
				...
				linkBuiltFiles(baseBuildPath, pathTail + "/" + name); // Recursively process next dir
			}
			else if (item.is_regular_file())
			{
				...
				if (fs::exists(baseNotesDir + "/build/.flat_notes/" + name) && itr != fileList.end() && itr->second->shouldBuild()) 
				{
